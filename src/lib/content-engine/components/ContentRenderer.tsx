...
import type { EnrichedBlock, EnrichedDocument } from '../types';
...
const headingRenderer = {
  heading(text: string, level: number, raw: string, number?: string) {
    const slug = raw
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/^-|-$/g, '');
    ...
    return `<${tagName} id="${slug}" data-heading-number="${number ?? ''}" class="notion-heading-level-${level}">${prefix}${text}</${tagName}>`;
  },
};
...
export function ContentRenderer({
  ...
  const renderBlock = useCallback((block: EnrichedBlock, index: number): React.ReactNode => {
    switch (block.type) {
      case 'heading': {
        ...
        const HeadingTag = `h${hLevel}` as 'h1' | 'h2' | 'h3' | 'h4' | 'h5' | 'h6';
        return (
          <HeadingTag
            key={index}
            id={block.id}
      ...
      case 'image': {
        if (!block.data || !('kind' in block.data)) return null;
        return <ImageBlock key={index} data={block.data as any} />;
      }
        ...
        {showToc && toc.length > 0 && (
          <div className="mb-8 p-4 rounded-lg border border-[var(--color-hairline)] bg-[var(--color-canvas-soft)]">
            ...
            <nav className="space-y-0.5">
              {renderTOCItems(toc, handleNavigate)}
            </nav>
          </div>
        )}
...
function renderTOCItems(items: TOCItem[], onNavigate: (id: string) => void, depth = 0): React.ReactNode {
    ...
    <ul className="list-none space-y-0.5">
      {items.map((item, i) => (
        <li key={i}>
          ...
          {item.children.length > 0 && renderTOCItems(item.children, onNavigate, depth + 1)}
        </li>
      ))}
    </ul>
  );
